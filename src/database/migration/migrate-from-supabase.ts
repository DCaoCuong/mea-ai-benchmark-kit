...
async function migrateComparisonRecords() {
    ...
    // Connect to SQLite
    const sqliteDbPath = process.env.SQLITE_DB_PATH || './data/db/medical_assistant.db';
    ...
    `;
        ...
        `);
                ...
                successCount++;
                console.log(`  ✓ Migrated record ${successCount}/${pgRecords.length}: ${pgRecord.id}`);
            } catch (error) {
                errorCount++;
        ...
        console.log('\n🔍 Verifying migration...');
        const sqliteCount = sqlite.prepare('SELECT COUNT(*) as count FROM comparison_records').get() as { count: number };
